...
Not every push builds. `vercel.json`'s `ignoreCommand` skips the build when a commit touches only `docs/`, the root `README.md`, `CLAUDE.md`, `AGENTS.md`, or `LICENSE` (compared against the branch's last successful deployment, `VERCEL_GIT_PREVIOUS_SHA`, or the commit's first parent when there is none, so a push whose last commit is docs-only still builds if an earlier commit in it touched code, and a commit after a failed build is retried). Nothing under `apps/` or `packages/` is ever skipped: the agent briefs in `packages/agent/briefs/*.md` are build inputs. A skipped production push leaves `main` on the previous deployment, which is correct because none of those files reach the site. A build-log-only commit costs nothing; before 2026-09-21 it cost a full build.
