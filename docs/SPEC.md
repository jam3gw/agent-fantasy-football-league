...
| `/sessions` | the newest 600 sessions across all teams and the reporter, filtered by team, status and kind in the browser with the filters mirrored into the URL, a hundred rows at a time; a "live now" panel for queued and running sessions; older sessions stay on the team pages |
