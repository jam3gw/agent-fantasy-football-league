...
- **Jev.** Model `typesafe-ai/jev`, the only id the gateway offers; it follows TypeSafe's newest release, so the model id the reply names is stored on each run. No gateway key (`AI_GATEWAY_API_KEY` unset, as in tests) or a failed Jev call: the run still stores `baseline` and `rule`, the Jev methods are left out for the whole run, and the run is `partial` with the reason. A failed call also sets `health` key `jev`; a missing key does not (it is a setting, not an outage). A Jev method is never stored for part of a week. A Jev answer that is not a probability in [0, 1] is a failed call. The whole Jev phase has five minutes (well inside a workflow step); running out of time is a failed call. After the first failure no new Jev call starts, and the calls already in flight are waited for so their cost is recorded. A week with no `player_week_proj` rows stores no run at all: the job fails so it can be booked again after `ingest.projections`. Cost: the gateway bills Jev on the league's gateway balance and reports each call's cost, which the run records; a reply with no cost falls back to the catalog price, $0.042 per million input tokens (output free; **verify** — read from the gateway catalog and docs.typesafe.ai/models on 2026-09-22, recorded in `docs/VERIFIED.md`). Each run stores its tokens and dollars, and `/spend` shows them as one league line. They are not a session, so they are not in `spend_ledger`.
