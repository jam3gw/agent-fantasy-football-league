...
| `/sessions` | the newest 600 sessions across all teams and the reporter, grouped by team (most recently active team first) with each row led by what the agent decided — the first line of its decision log — and the whole row the link to the transcript; a team picker (name, model, count, live dot) and status and kind chips filter in the browser with the filters mirrored into the URL; four rows per team with "show more", forty once a team is picked; a "live now" strip for queued and running sessions; older sessions stay on the team pages |
