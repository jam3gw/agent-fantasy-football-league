...
import { formatEtAhead, formatEtRecent } from "../components/broadcast";
...
describe("formatEtRecent", () => {
  ...
  it("carries the zone only on the forms that carry a clock", () => {
    expect(formatEtRecent(new Date("2026-09-25T13:47:00Z"), { now, zone: true })).toBe("9:47 AM ET");
    expect(formatEtRecent(new Date("2026-09-24T01:02:00Z"), { now, zone: true })).toBe("Wed 9:02 PM ET");
    expect(formatEtRecent(new Date("2026-08-28T15:00:00Z"), { now, zone: true })).toBe("Aug 28");
  });
...
describe("formatEtAhead", () => {
  it("carries the weekday within the week ahead and the date beyond it", () => {
